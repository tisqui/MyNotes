...
/**
* from udemy.com course https://www.udemy.com/android-marshmallow-java-app-development-course/
 */
final public class GDActions {
    private GDActions() {
    }
    ...
    /**
     * Initialize Google Drive API when called from NoteDetailActivity
     * @param ctx context
    ...
    /**
     * Initialize Google Drive API when called from NoteActivity
     * @param ctx - context
     * @param email - google account
     */
     ...
     * @param bOn - defining the action - connect or disconnect. true - connect, false - disconnect
     */
    ...
    /**
     * Check if connected to google API
     ...
     * @param prId recourse id
     * @param titl file name
     * @param mime match criteria
     * @return array list of google files
     */
    ...
    static DriveId create(DriveId prId, String titl, String mime, byte[] buf) {
        ...
        if (buf != null) {
            ...
            meta = new MetadataChangeSet.Builder().setTitle(titl).setMimeType(GDUtilities.MIME_FLDR).build();
    ...
    static DriveId createTreeGDAA(String root, String titl, byte[] buf) {
        ...
        if (dId != null) {
            dId = findOrCreateFolder(dId, GDUtilities.titl2Month(titl));
            if (dId != null) {
                return create(dId, titl + GDUtilities.JPEG_EXT, GDUtilities.MIME_JPEG, buf);
            }
        }
        return null;
    }
    ...
    private static DriveId findOrCreateFolder(DriveId prnt, String titl) {
        ArrayList<GF> gfs = search(prnt, titl, GDUtilities.MIME_FLDR);
        if (gfs.size() > 0) {
        ...
        if (titl != null && isConnected()) {
            ...
            if (buf != null) {
                if (mime != null) {
                    ...
                    java.io.File jvFl;
                    try {
                        jvFl = GDUtilities.bytes2File(buf,
                                java.io.File.createTempFile(GDUtilities.TMP_FILENM, null, GDUtilities.acx.getCacheDir()));
                ...
                meta.setMimeType(GDUtilities.MIME_FLDR);
                try {
